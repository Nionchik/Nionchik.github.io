...
pygame.display.set_caption("Игра с кубиками")
...
bg = pygame.image.load("montains.jpg")
...
while run:
    ...
    if keys[pygame.K_LEFT] and x > 5:
        ...
        right = False
        lastMove = "left"
    elif keys[pygame.K_RIGHT] and x < 490 - width:
        x += speed
        left = False
        right = True
        lastMove = "right"
    ...
    if not(isJump):
        if keys[pygame.K_UP] and y > 5:
            y -= speed
        if keys[pygame.K_DOWN] and y < 490 - height:
            y += speed
